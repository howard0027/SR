...
class ShiftConv2d0(nn.Module):
    def __init__(self, inp_channels, out_channels):
        ...
        mask = nn.Parameter(torch.zeros((self.out_channels, self.inp_channels, 3, 3)), requires_grad=False)#.to(torch.device('cuda'))
...
class ShiftConv2d1(nn.Module):
    def __init__(self, inp_channels, out_channels):
        ...
        self.inp_channels = inp_channels
        self.out_channels = out_channels
        #print(f'self.in = {self.inp_channels}, self.out = {self.out_channels}')
        self.weight = nn.Parameter(torch.zeros(inp_channels, 1, 3, 3), requires_grad=False)#.to(torch.device('cuda'))
        self.n_div = 5
        g = inp_channels // self.n_div
